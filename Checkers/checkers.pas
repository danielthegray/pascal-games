...
label
	StartOfGame, ReturnToGame, CoordInput2;
...
begin
	  ...
	  OutTextXY(13, 350, 'Use your mouse to select and move your pieces.');
	  OutTextXY(13, 365, 'Right click or move in an invalid way to cancel your move.');
	  OutTextXY(13, 380, 'Player A and Player B are the default names.');
	  OutTextXY(13, 395, 'To change them, click on Input Players'' Names.');
	  OutTextXY(13, 410, 'The order will be picked randomly, and you will be allowed');
	  OutTextXY(13, 425, 'to reverse the order.');
	  SetColor(White);
	  OutTextXY(18, 443, 'Press any key to play.');
	  ClearKeybBuffer;
	  ReadKey;
end;
...
begin
	...
	repeat
		...
		SetTextStyle(SmallFont, HorizDir, 6);
		SetColor(LightRed);
		OutTextXY(498, 301, 'Click for New Game');
		Rectangle(400, 292, 590, 315);
		SetColor(Green);
		OutTextXY(501, 351, 'Instructions');
		Rectangle(440, 342, 560, 365);
		SetColor(LightGreen);
		...
		repeat
			...
			if Left then
			begin
				if (PointerX >= 440) and (PointerX <= 560)
				and (PointerY >= 342) and (PointerY <= 365) then
				begin
				...
				if (PointerX >= 475) and (PointerX <= 565)
				...
				and (PointerY >= 292) and (PointerY <= 315) then
				begin
					repeat
						ReadMouse;
					until not Left;
